...
namespace Crazy.Cards.Web.Controllers
{
    public class CardOffersController : ApiController
    {
        ...
        {
            InMemoryCardsRepository r = new InMemoryCardsRepository();
            var cardOffers =  r.GetCards(model.EmploymentStatus, model.AnnualIncome);
        ...
        {
            List<OfferViewModel> a = new List<OfferViewModel>();
            foreach (var r in cardOffers)
            {
                a.Add(new OfferViewModel() {
